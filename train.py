...
from compose_models import RawNetWithFC, get_inc_ssdnet, get_rawnet2, get_ssdnet, get_wav2vec2_model
...
from utils import batch_audio_to_mel, batch_mel_to_audio, get_transciption_loss, transcribe_audio
...
def train(epoch):
    ...
    for index, train_sample in enumerate(progress_bar):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16):  # Enable Mixed Precision
            ...
            forged_transciption = transcribe_audio(forged,t_processor_1,t_model_1,device)
            attacked_transciption = transcribe_audio(attacked,t_processor_1,t_model_1,device)
            if index == 0:
            ...
            g_loss = per_loss + adv_loss + s1_w * c1_loss+ t1_weight*t_loss  # s2_w*c2_loss + 
...
def test(epoch=0):
    ...
    for test_sample in progress_bar:
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16):  # Enable Mixed Precision
            ...
            real_acc.append(cal_acc(y_real, real,cl_model1))
            fake_acc.append(cal_acc(y_fake, forged,cl_model1))
            af_acc.append(cal_acc(y_fake, fake,cl_model1))
...
def main():
    ...
    for epoch in range(args.nEpochs):
        ...
        r_acc, f_acc, af_acc = test(epoch) # , r2_acc, f2_acc, af2_acc
