...
from train_data_loader_combined import CombinedDATAReader
...
def train_model(model, criterion, optimizer , train_loader, val_loader, num_epochs, save_path,save_every):
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Epoch {epoch+1}/{num_epochs} - Loss: {epoch_loss:.4f} - Accuracy: {100 * correct / total:.2f}% - "
              f"Val Accuracy: {val_accuracy:.2f}% - Val Loss: {val_loss:.4f} - Val AUC: {auc:.4f} - Val EER: {eer:.4f}")

        # Save the best model based on AUC
        if (epoch+1) % save_every == 0 and auc > best_auc and auc < 1:   # exceptional case, auc =1
            best_auc = auc
            best_eer = eer
...
def get_model(model_name, device):
    ...
    if model_name.lower() == 'aasist':
        ...
        # SSDNet Model
        model = SSDNet1D().to(device)    # SSDNet1D_S, DilatedNet_S, SSDNet1D_L, DilatedNet_L
    elif model_name.lower() == 'inc_ssdnet':
        # SSDNet Model
        model = DilatedNet().to(device)    # SSDNet1D_S, DilatedNet_S, SSDNet1D_L, DilatedNet_L
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', '-b', type=int, default=64, help='')
    parser.add_argument('--num_workers', '-w', type=int, default=8, help='')
    ...
    dev_loader = data.DataLoader(test_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True, drop_last=True)

    model_list = ["rawnet3","rawnet2", "rawboost","resnet1d", "msresnet", "ssdnet", "inc_ssdnet", "ssdnet_small", "ssdnet_large", "inc_ssdnet_small", "inc_ssdnet_large" ]

    model_name = "resnet1d" # models_list = "aasist", "rawnet3", "rawnet2", "rawboost", "ssdnet", "inc_ssdnet", "resnet1d", "msresnet", "acnn1d"
    save_every = 5
    device_id = 7
    ...
    model_save_path = f"./weights/{model_name}/best_{model_name}_Combined"
