...
class SSDNet1D_L(nn.Module):  # Res-TSSDNet
    def __init__(self):
        super().__init__()
        ...
        self.RSM4 = RSM1D(channels_in=128, channels_out=256)
        self.RSM5 = RSM1D(channels_in=256, channels_out=256)
    ...
    def forward(self, x):
        ...
        x = self.RSM5(x)
        # x = F.max_pool1d(x, kernel_size=x.shape[-1])
        x = F.adaptive_max_pool1d(x, output_size=1) #F.max_pool1d(x, kernel_size=375)
...
class DilatedNet_L(nn.Module):  # Inc-TSSDNet
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv1d(in_channels=1, out_channels=16, kernel_size=7, padding=3, bias=False)
        self.bn1 = nn.BatchNorm1d(16)
        ...
        self.DCM4 = DilatedCovModule(channels_in=128, channels_out=256)
        self.DCM5 = DilatedCovModule(channels_in=256, channels_out=256)
    ...
    def forward(self, x):
        ...
        x = F.max_pool1d(self.DCM4(x), kernel_size=4)
        x = self.DCM5(x)
        # x = F.max_pool1d(self.DCM5(x), kernel_size=375)
        x = F.adaptive_max_pool1d(x, output_size=1) #F.max_pool1d(x, kernel_size=375)
...
if __name__ == '__main__':
    Res_TSSDNet = SSDNet1D_L()
    Res_TSSDNet_2D = SSDNet2D()
    Inc_TSSDNet = DilatedNet_L()
