...
class SSDNet1D_S(nn.Module):  # Res-TSSDNet
    def __init__(self):
        super().__init__()
        ...
        self.RSM4 = RSM1D(channels_in=64, channels_out=128)
...
class DilatedNet_S(nn.Module):  # Inc-TSSDNet
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv1d(in_channels=1, out_channels=16, kernel_size=7, padding=3, bias=False)
        self.bn1 = nn.BatchNorm1d(16)
        ...
        self.DCM4 = DilatedCovModule(channels_in=64, channels_out=128)
...
if __name__ == '__main__':
    Res_TSSDNet = SSDNet1D_S()
    Res_TSSDNet_2D = SSDNet2D()
    Inc_TSSDNet = DilatedNet_S()
